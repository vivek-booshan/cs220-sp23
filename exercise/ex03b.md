...
```bash
git clone https://github.com/jhu-ip/cs220-summer22-public.git
```
3.	Type:
``` bash
...
If you did not successfully complete the above steps, ask of a member of the course staff and/or post on <a href="http://piazza.com/jhu/summer2022/en601220" target="_blank">Piazza</a>.
...
```bash
cp ~/cs220-summer22-public/exercises/ex02/hello_world.c .
```
...
1.	While you do not want to remove or even modify the `cs220-summer22-public` folder (that is, your copy of our public class repo), you will no longer need to keep the temporary directory named `temp` that we created today, or any of the files you put inside it.  To remove the `temp` directory, first navigate to your home directory (that is, the top-level folder in your account) by typing `cd ~` or simply `cd`.
